...
_lr_productions = [
  ...
  ('start -> assign','start',1,'p_start','plyYacc.py',34),
  ('start -> fLoop stmt','start',2,'p_start','plyYacc.py',35),
  ('start -> fLoop stmt start','start',3,'p_start','plyYacc.py',36),
  ('start -> assign start','start',2,'p_start','plyYacc.py',37),
  ('start -> assign fLoop start','start',3,'p_start','plyYacc.py',38),
  ('start -> stmt','start',1,'p_start','plyYacc.py',39),
  ('stmt -> PRINT VAR','stmt',2,'p_stmt','plyYacc.py',50),
  ('assign -> factor EQUAL expr','assign',3,'p_assign','plyYacc.py',63),
  ('expr -> expr PLUS term','expr',3,'p_expr','plyYacc.py',77),
  ('expr -> expr MINUS term','expr',3,'p_expr','plyYacc.py',78),
  ('expr -> term','expr',1,'p_expr','plyYacc.py',79),
  ('term -> term MULTIPLY factor','term',3,'p_term','plyYacc.py',113),
  ('term -> term DIVIDE factor','term',3,'p_term','plyYacc.py',114),
  ('term -> factor','term',1,'p_term','plyYacc.py',115),
  ('factor -> VAR','factor',1,'p_factor','plyYacc.py',148),
  ('factor -> NUMBER','factor',1,'p_factor','plyYacc.py',149),
  ('fLoop -> FOR inner','fLoop',2,'p_fLoop','plyYacc.py',157),
  ('inner -> PARANOPEN VAR IN rangeSpec PARANCLOSE COLON','inner',6,'p_inner','plyYacc.py',162),
  ('inner -> VAR IN rangeSpec COLON','inner',4,'p_inner','plyYacc.py',163),
  ('rangeSpec -> RANGE PARANOPEN expr COMMA expr PARANCLOSE','rangeSpec',6,'p_rangeSpec','plyYacc.py',175),
]
